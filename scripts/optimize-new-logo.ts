...
const inputPath = "attached_assets/new-logo-v2.png";
const outputDir = "attached_assets/optimized";

async function optimizeLogo() {
  console.log('🦁 Optimizing Jack\'s Lounge logo (v2)...\n');
  ...
  // Target dimensions - back to original size (h-14 = 56px height)
  // New logo is horizontal format, calculate width based on aspect ratio
  const desktopHeight = 56; // h-14 = 56px (original size)
  const aspectRatio = metadata.width! / metadata.height!;
  const desktopWidth = Math.round(desktopHeight * aspectRatio);

  // Mobile version (same height, proportional width)
